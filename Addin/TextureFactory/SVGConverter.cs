...
namespace TexFac.Universal
{
    ...
    public class SvgConverter
    {
        ...
        public static Bitmap ConvertSvgContentToBitmap(
            string svgContent,
            int width = 0,
            int height = 0,
            Dictionary<string, string> colorReplacements = null)
        {
            ...
            svgConverter.colorReplacements = colorReplacements ?? new Dictionary<string, string>();
            
            svgConverter.svgDoc = new XmlDocument();
            svgConverter.svgDoc.LoadXml(svgContent);
            
            XmlElement svgElement = svgConverter.svgDoc.DocumentElement;
            int svgWidth = width;
            ...
            {
                float[] values = svgConverter.ParseFloatArray(viewBox);
                if (values.Length == 4)
                {
                    viewBoxX = values[0];
                    viewBoxY = values[1];
            ...
            {
                originalWidth = svgConverter.GetDimension(svgElement, "width", 100);
                originalHeight = svgConverter.GetDimension(svgElement, "height", 100);
            }
            ...
            svgConverter.scaleX = (float)svgWidth / originalWidth;
            svgConverter.scaleY = (float)svgHeight / originalHeight;
            ...
            using (Graphics g = Graphics.FromImage(bitmap))
            {
                ...
                {
                    g.TranslateTransform(-viewBoxX * svgConverter.scaleX, -viewBoxY * svgConverter.scaleY);
                }
                
                svgConverter.RenderElement(g, svgElement);
            }
        ...
        public static byte[] ConvertSvgContentToBytes(
            string svgContent,
            int width = 0,
            int height = 0,
            Dictionary<string, string> colorReplacements = null)
        {
            using (Bitmap bitmap = ConvertSvgContentToBitmap(svgContent, width, height, colorReplacements))
            {

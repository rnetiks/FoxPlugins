...
public class HarmonyPatchInspector : BaseUnityPlugin
{
    public const string PLUGIN_GUID = "com.fox.harmonypatchinspector";
    public const string PLUGIN_NAME = "Harmony Patch Inspector";
    ...
    private Dictionary<string, List<PatchMethodInfo>> originalPatchStates = new Dictionary<string, List<PatchMethodInfo>>();

    private enum ILViewMode
    {
        Original,
        PatchMethods,
        PatchDetails
    ...
    private class ILViewerWindow
    {
        public int WindowId;
        public Rect WindowRect;
        public MethodBase Method;
        public string OriginalILCode;
        public List<PatchMethodInfo> PatchMethods;
        public string PatchDetailsCode;
        ...
        public ILViewerWindow(int windowId, MethodBase method, string originalIL, List<PatchMethodInfo> patchMethods, string patchDetails)
        {
            WindowId = windowId;
            Method = method;
            OriginalILCode = originalIL;
            PatchMethods = patchMethods;
            PatchDetailsCode = patchDetails;
    ...
    private void OnGUI()
    {
        if (showMenu)
        {
            windowRect = GUI.Window(12345, windowRect, DrawWindow, "Harmony Patch Inspector");
        ...
        foreach (var kvp in ilViewerWindows)
        {
            if (kvp.Value.IsOpen)
            {
                kvp.Value.WindowRect = GUI.Window(kvp.Value.WindowId, kvp.Value.WindowRect,
                    (id) => DrawILViewerWindow(id, kvp.Value), $"IL Code: {kvp.Value.Method.DeclaringType?.Name}.{kvp.Value.Method.Name}");
                if (kvp.Value.WindowRect.Contains(Event.current.mousePosition))
                    Input.ResetInputAxes();
            }
            else
            {
    ...
    private void DrawWindow(int windowID)
    {
        ...
        GUILayout.Label("Examples: 'OnClick', 'method:OnClick class:AddButtonCtrl', 'namespace:Studio'");
        ...
        {
            GUILayout.Label($"Found {foundPatches.Count} patches:", GUI.skin.label);

            scrollPosition = GUILayout.BeginScrollView(scrollPosition, GUILayout.Height(440));
            ...
            foreach (var patch in foundPatches)
            {
                ...
                GUI.color = bgColor;
                GUILayout.Label($"Method: {patch.TargetType}.{patch.MethodName}", GUI.skin.label);
        ...
        {
            GUILayout.Label("No patches found for the specified search term.");
        }
        else
        {
    ...
    private void SearchPatches()
    {
        ...
        try
        {
            ...
            var harmony = Harmony.CreateAndPatchAll(typeof(HarmonyPatchInspector));
            ...
            foundPatches = foundPatches.OrderBy(p => p.TargetType).ThenBy(p => p.MethodName).ToList();
        }
        catch (Exception ex)
        {
            // ignored
        ...
        if (input.Contains(":"))
        {
            ...
            foreach (var part in parts)
            {
                if (part.StartsWith("method:", StringComparison.OrdinalIgnoreCase))
                    ...
                    criteria.FreeText = part;
            }
        }
        else
        {

            criteria.FreeText = input;
    ...
    private bool MatchesSearchCriteria(MethodBase method, SearchCriteria criteria)
    {
        if (criteria.IsStructured)
        {
            ...
            if (!string.IsNullOrEmpty(criteria.FreeText))
            {
                ...
                string searchTerm = criteria.FreeText.ToLower();
    ...
    private void AddPatchesToList(Patch[] patches, MethodBase targetMethod, string patchType)
    {
        ...
        if (originalPatchStates.ContainsKey(methodKey))
        {
            var storedPatches = originalPatchStates[methodKey];
            foreach (var storedPatch in storedPatches)
            {
                if (!storedPatch.IsEnabled && storedPatch.PatchType == patchType)
                {

                    bool alreadyAdded = foundPatches.Any(fp =>
                        fp.TargetMethod == targetMethod &&
                        fp.PatchType == patchType &&
                        fp.PatcherNamespace == storedPatch.PatcherNamespace);
    ...
    private void OpenILViewer(MethodBase method)
    {
        ...
        try
        {
            ...
            string originalIL = DisassembleMethod(method);
            ...
            if (originalPatchStates.ContainsKey(methodKey))
            {
                ...
                patchMethods = originalPatchStates[methodKey];
            ...
            else
            {
                ...
                originalPatchStates[methodKey] = patchMethods;

            }


            string patchDetails = GeneratePatchDetails(method, patchMethods);

            var window = new ILViewerWindow(nextWindowId++, method, originalIL, patchMethods, patchDetails);
    ...
    private void DrawILViewerWindow(int windowId, ILViewerWindow window)
    {
        ...
        GUILayout.Label($"Method: {window.Method.DeclaringType?.FullName}.{window.Method.Name}", GUI.skin.label);
        ...
        bool originalSelected = window.CurrentView == ILViewMode.Original;
        if (originalSelected) GUI.color = Color.yellow;
        ...
        if (originalSelected) GUI.color = Color.white;
        ...
        bool patchDetailsSelected = window.CurrentView == ILViewMode.PatchDetails;
        if (patchDetailsSelected) GUI.color = Color.yellow;
        if (GUILayout.Button("Patch Details", GUILayout.Height(30)))
        {
            window.CurrentView = ILViewMode.PatchDetails;
            window.ScrollPosition = Vector2.zero;
        }
        if (patchDetailsSelected) GUI.color = Color.white;
        ...
        switch (window.CurrentView)
        {
            ...
            case ILViewMode.PatchDetails:
                DrawPatchDetailsView(window);
                break;
        }
    ...
    {
        GUILayout.Label("Original Method IL Code:", GUI.skin.label);
        window.ScrollPosition = GUILayout.BeginScrollView(window.ScrollPosition, GUILayout.Height(450));

        GUIStyle codeStyle = GetCodeStyle();
        GUILayout.TextArea(window.OriginalILCode, codeStyle);
    ...
    private void DrawPatchManagerView(ILViewerWindow window)
    {
        ...
        GUILayout.Label("Patches:", GUI.skin.label);
        ...
        for (int i = 0; i < window.PatchMethods.Count; i++)
        {
        ...
        if (window.SelectedPatchIndex >= 0 && window.SelectedPatchIndex < window.PatchMethods.Count)
        {
            ...
            GUIStyle codeStyle = GetCodeStyle();
            GUILayout.TextArea(selectedPatch.ILCode, codeStyle);
    ...
    private void DrawPatchDetailsView(ILViewerWindow window)
    {
        GUILayout.Label("Patch Details & Information:", GUI.skin.label);
        window.ScrollPosition = GUILayout.BeginScrollView(window.ScrollPosition, GUILayout.Height(450));

        GUIStyle codeStyle = GetCodeStyle();
        GUILayout.TextArea(window.PatchDetailsCode, codeStyle);
    ...
    private GUIStyle GetCodeStyle()
    {
        ...
        codeStyle.font = Resources.GetBuiltinResource<Font>("Arial.ttf");
    ...
    private string GeneratePatchDetails(MethodBase targetMethod, List<PatchMethodInfo> patchMethods)
    {
        var sb = new StringBuilder();

        sb.AppendLine($"Target Method: {targetMethod.DeclaringType?.FullName}.{targetMethod.Name}");
        sb.AppendLine($"Total Patches: {patchMethods.Count}");
        sb.AppendLine();

        if (patchMethods.Count == 0)
        {
            sb.AppendLine("No patches found for this method.");
            return sb.ToString();
        }

        var groupedPatches = patchMethods.GroupBy(p => p.PatchType).OrderBy(g => GetPatchTypeOrder(g.Key));

        foreach (var group in groupedPatches)
        {
            sb.AppendLine($"═══ {group.Key} Patches ({group.Count()}) ═══");
            sb.AppendLine();

            var sortedPatches = group.OrderByDescending(p => p.Priority).ToList();

            for (int i = 0; i < sortedPatches.Count; i++)
            {
                var patch = sortedPatches[i];

                sb.AppendLine($"  [{i + 1}] {patch.PatchMethod.DeclaringType?.Name}.{patch.PatchMethod.Name}");
                sb.AppendLine($"      Namespace: {patch.PatcherNamespace}");
                sb.AppendLine($"      Assembly: {patch.PatchMethod.DeclaringType?.Assembly?.GetName().Name ?? "Unknown"}");
                sb.AppendLine($"      Priority: {patch.Priority}");
                sb.AppendLine($"      Harmony ID: {patch.HarmonyId ?? "Unknown"}");
                sb.AppendLine($"      Parameters: {GetMethodParameters(patch.PatchMethod)}");
                sb.AppendLine($"      Return Type: {GetReturnType(patch.PatchMethod)}");
                ...
                try
                {
                    ...
                    {
                        sb.AppendLine($"      File: {assembly.Location}");
                    }
                }
                catch { }
        ...
        sb.AppendLine();
        sb.AppendLine("WARNING: Disabling patches may cause instability");
        sb.AppendLine("or unexpected behavior. Use for debugging only!");

        return sb.ToString();
    }

    private int GetPatchTypeOrder(string patchType)
    {
        switch (patchType)
        {
            case "Prefix": return 1;
            case "Postfix": return 2;
            case "Transpiler": return 3;
            case "Finalizer": return 4;
            default: return 5;
        }
    }
    ...
    private string GetMethodParameters(MethodBase method)
    {
        try
        {
            ...
            var paramStrings = parameters.Select(p => $"{p.ParameterType?.Name ?? "Unknown"} {p.Name}");
    ...
    private string GetReturnType(MethodBase method)
    {
        try
        {
            if (method is MethodInfo methodInfo)
                return methodInfo.ReturnType?.Name ?? "Unknown";
            return "void";
        }
        catch
        {

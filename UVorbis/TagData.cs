...
namespace NVorbis
{
    ...
    {
        static IReadOnlyList<string> s_emptyList = new ReadOnlyListWrapper<string>(new List<string>());

        Dictionary<string, List<string>> _tags;
        IReadOnlyDictionary<string, IReadOnlyList<string>> _allCache;
        ...
        public IReadOnlyList<string> GetTagMulti(string key)
        {
            ...
            {
                return new ReadOnlyListWrapper<string>(values);
            }
            return s_emptyList;
        }
        ...
        public IReadOnlyDictionary<string, IReadOnlyList<string>> All
        {
            get
            {
                if (_allCache == null)
                {
                    ...
                    {
                        dict[kvp.Key] = new ReadOnlyListWrapper<string>(kvp.Value);
                    }
                    _allCache = new ReadOnlyDictionaryWrapper<string, IReadOnlyList<string>>(dict);
                }
                return _allCache;
            }
        }

...
namespace NVorbis
{
    class Codebook : ICodebook
    {
        ...
        public void Init(IPacket packet, IHuffman huffman)
        {
            ...
            if (chkVal != 0x564342UL) throw new Exception("Book header had invalid signature!");
        ...
        private void InitTree(IPacket packet, IHuffman huffman)
        {
            ...
            if ((_maxBits = maxLen) > -1)
            {
                ...
                if (!ComputeCodewords(sparse, codewords, codewordLengths, _lengths, Entries, values)) throw new Exception();

                var valueList = values != null ? (IReadOnlyList<int>)new ReadOnlyListWrapper<int>(values) : (IReadOnlyList<int>)FastRange.Get(0, codewords.Length);

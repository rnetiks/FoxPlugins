...
namespace NVorbis.Ogg
{
    internal class Packet : DataPacket
    {
        ...
        internal Packet(IReadOnlyList<int> dataParts, IPacketReader packetReader, ArraySegment<byte> initialData)
        {
        ...
        protected override int TotalBits => (_dataCount + _data.Count) * 8;

        protected override int ReadNextByte()
        {
            if (_dataIndex == _dataParts.Count) return -1;

            var b = _data.Array[_data.Offset + _dataOfs];
            ...
            if (++_dataOfs == _data.Count)
            {
                ...
                {
                    _data = new ArraySegment<byte>(new byte[0], 0, 0);
                }
            }

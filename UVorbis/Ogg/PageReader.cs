...
namespace NVorbis.Ogg
{
    class PageReader : PageReaderBase, IPageData
    {
        ...
        ArraySegment<byte>[] _packets;

        [ThreadStatic]
        private static int _lockThreadId;
        ...
        private static ArraySegment<byte>[] ReadPackets(int packetCount, byte[] segBuf, int segOfs, int segCount, byte[] dataBuf, int dataOfs, int dataCount)
        {
            ...
            {
                var seg = segBuf[segOfs + i];
                size += seg;
                if (seg < 255)
                {
                    if (size > 0)
                    {
                        list[listIdx++] = new ArraySegment<byte>(dataBuf, dataOfs + dataIdx, size);
                        dataIdx += size;
                    }
                    size = 0;
                }
        ...
        {
            return _lockThreadId == Thread.CurrentThread.ManagedThreadId && Monitor.TryEnter(_readLock, 0) ? ReleaseTryEnter() : false;
        }

        private bool ReleaseTryEnter()
        {
            // We successfully re-entered the lock via TryEnter, so release that extra entry
        ...
        {
            if (_lockThreadId == Thread.CurrentThread.ManagedThreadId)
            {
                _lockThreadId = 0;
                Monitor.Exit(_readLock);
                return true;
        ...
        protected override bool AddPage(int streamSerial, byte[] pageBuf, bool isResync)
        {
            ...
            _packets = ReadPackets(PacketCount, pageBuf, 27, pageBuf[26], pageBuf, 27 + pageBuf[26], pageBuf.Length - 27 - pageBuf[26]);

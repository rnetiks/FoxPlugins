...
namespace NVorbis.Ogg
{
    ...
    {
        private struct PreviousPageInfo
        {
            public long LastPageGranulePos;
            public int LastPagePacketLength;
            public int FirstRealPacket;
        }

        private struct TargetPageInfo
        {
            public long[] Gps;
            public long EndGP;
        }
        ...
        private PreviousPageInfo GetPreviousPageInfo(int pageIndex, GetPacketGranuleCount getPacketGranuleCount)
        {
            if (pageIndex > 0)
            {
                ...
                if (_reader.GetPage(pageIndex - 1, out var lastPageGranulePos, out _, out _, out var isContinued, out var lastPacketCount, out _))
                {
                    if (pageIndex > _reader.FirstDataPageIndex)
                    {
                        ...
                        if (lastPacket == null)
                        {
                            throw new Exception("Could not find end of continuation!");
                        }
                    ...
                    return new PreviousPageInfo { LastPageGranulePos = lastPageGranulePos, LastPagePacketLength = lastPagePacketLength, FirstRealPacket = isContinued ? 1 : 0 };
                }
                throw new Exception("Could not get preceding page?!");
            }
            else
            {
                return new PreviousPageInfo { LastPageGranulePos = 0, LastPagePacketLength = 0, FirstRealPacket = 0 };
            }
        }

        private TargetPageInfo GetTargetPageInfo(int pageIndex, int firstRealPacket, int lastPagePacketLength, GetPacketGranuleCount getPacketGranuleCount)
        {
            ...
            {
                throw new Exception("Could not get found page?!");
            }
            ...
            return new TargetPageInfo { Gps = gps, EndGP = endGP };
        }
        ...
        private int FindPacket(int pageIndex, long[] gps, long endGP, long lastPageGranulePos, int lastPagePacketLength, ref long granulePos)
        {
            ...
            if (endGP != lastPageGranulePos)
            {
                ...
                {
                    // unknown error...
                    throw new Exception($"GranulePos mismatch: Page {pageIndex}, expected {lastPageGranulePos}, calculated {endGP}");
                }
            }
            ...
            throw new Exception("Could not find seek packet?!");
        }
        ...
        private int FindPacket(int pageIndex, int preRoll, ref long granulePos, GetPacketGranuleCount getPacketGranuleCount)
        {
            ...
            var prevInfo = GetPreviousPageInfo(pageIndex, getPacketGranuleCount);

            // now get the info on the target page
            var targetInfo = GetTargetPageInfo(pageIndex, prevInfo.FirstRealPacket, prevInfo.LastPagePacketLength, getPacketGranuleCount);

            // finally figure out which packet in our known info we need to use
            var packetIndex = FindPacket(pageIndex, targetInfo.Gps, targetInfo.EndGP, prevInfo.LastPageGranulePos, prevInfo.LastPagePacketLength, ref granulePos);

            // then apply the preRoll (but only if we're not seeking into the first packet, which is its own preRoll)
            if (targetInfo.EndGP > 0 || packetIndex > 1)
            {
                packetIndex -= preRoll;
            }
            return packetIndex;
        ...
        private Packet CreatePacket(ref int pageIndex, ref int packetIndex, bool advance, long granulePos, bool isResync, bool isContinued, int packetCount, int pageOverhead)
        {
            ...
            var packet = new Packet(new ReadOnlyListWrapper<int>(pktList), this, firstPacketData)
            {
                IsResync = isResync,
            };
        ...
        ArraySegment<byte> IPacketReader.GetPacketData(int pagePacketIndex)
        {

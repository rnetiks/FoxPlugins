...
namespace NVorbis.Ogg
{
    class ForwardOnlyPacketProvider : DataPacket, IForwardOnlyPacketProvider
    {
        private struct PageEntry
        {
            ...
            public PageEntry(byte[] buf, bool isResync)
            {
                Buf = buf;
                IsResync = isResync;
            }
        ...
        private readonly Queue<PageEntry> _pageQueue = new Queue<PageEntry>();
        ...
        public bool AddPage(byte[] buf, bool isResync)
        {
            ...
            _pageQueue.Enqueue(new PageEntry(buf, isResync));
            return true;
        }
        ...
        private bool GetPacket()
        {
            ...
            var packetBuf = new ArraySegment<byte>(pageBuf, dataStart, dataLen);
            dataStart += dataLen;
            ...
            else
            {
                while (isCntd && packetIndex == 27 + pageBuf[26])
                {
                    if (ReadNextPage(out pageBuf, out isResync, out dataStart, out packetIndex, out isCont, out isCntd) && !isResync && isCont)
                    {
                        ...
                        var newBuf = new byte[prevBuf.Count + contSz];
                        packetBuf = new ArraySegment<byte>(newBuf, 0, newBuf.Length);
                        prevBuf.CopyTo(packetBuf);
                        (new ArraySegment<byte>(pageBuf, dataStart, contSz)).CopyTo(packetBuf.Slice(prevBuf.Count));
        ...
        {
            _packetBuf = new ArraySegment<byte>(new byte[0], 0, 0);
            base.Done();
        }

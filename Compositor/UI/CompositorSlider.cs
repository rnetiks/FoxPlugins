...
namespace DefaultNamespace
{
    public class CompositorSlider
    {
        ...
        public CompositorSlider(float value, float minValue, float maxValue, float defaultValue, string label = "", string format = "F2")
        {
            _value = value;
            _minValue = minValue;
            _maxValue = maxValue;
        ...
        public float Draw(Rect rect)
        {
            ...
            if (currentEvent.type == EventType.MouseDrag && GUIUtility.hotControl == controlID)
            {
                ...
                if (dragDistance > DRAG_THRESHOLD)
                {
                    ...
                    float sensitivity = (_maxValue - _minValue) / rect.width * 2f;

                    float newValue = _dragStartValue + dragDelta * sensitivity;

                    if (currentEvent.shift)
                    {
                        newValue = Mathf.Round(newValue * 10) / 10;
                    }
        ...
        private void DrawSlider(Rect rect, bool isHovering, bool isActive)
        {
            ...
            GUI.DrawTexture(rect, GUIUtils.GetColorTexture(backgroundColor));

            if (_value >= _minValue && _value <= _maxValue)
            {
                Rect fillRect = new Rect(rect.x, rect.y, rect.width * normalizedValue, rect.height);
                GUI.DrawTexture(fillRect, GUIUtils.GetColorTexture(fillColor));
            }
            else
            {
                Rect indicatorRect = new Rect(rect.x + 2, rect.y + 2, rect.width - 4, rect.height - 4);
                GUI.DrawTexture(indicatorRect, GUIUtils.GetColorTexture(new Color(fillColor.r, fillColor.g, fillColor.b, 0.3f)));
            }

...
namespace Autumn
{
    ...
    public static class Helper
    {
        ...
        public static Vector2 MovePoint(int x, int y, float distance, float angle, bool useDegree = true)
        {
            if (useDegree) angle = Mathf.Deg2Rad * angle;
            var x1 = x + distance * Mathf.Cos(angle);
            var y1 = y + distance * Mathf.Sin(angle);
            return new Vector2(x1, y1);
        }

        public static Vector2 MovePoint(this Vector2 point, float distance, float angle, bool useDegree = true)
        {
            if (useDegree) angle = angle * Mathf.Deg2Rad;
            var x1 = point.x + distance * Mathf.Cos(angle);
            var y1 = point.y + distance * Mathf.Sin(angle);
            return new Vector2(x1, y1);
        }
